...
class ActionSteps extends BaseSteps {
  ...
  Given("^I start a (.*)$") { (journey: String) =>
    ...
    journey match {
      case "journey" => driver.navigate.to(TestOnlyClearAttemptsPage.url)
  ...
  When("^I click to (.*)$") { (element: String) =>
    element match {
      ...
      case "choose another way to get my refund" | "claim your refund by bank transfer"             => clickById("choose-another-method-link")
  ...
  When("^I click the link (.*)$") { (link: String) =>
    link match {
      ...
      case "contact us after lockout"                   => clickByCssSelector("contact-hmrc")
      case "contact us from lockout"                   => clickById("p > a:nth-child(2)")
